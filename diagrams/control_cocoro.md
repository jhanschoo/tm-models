...
The **control cocoro** dynamically emulates an immutable version of the control tape used in the $O(n \lg n)$ efficient UTM. It uses three tapes, with their roles and invariants as follows. We use $Z, Z_{0}, Z_{1}$, ... to refer to the zones from the simulated head to the current head, when we do not want to specify whether it is on the left half-tape or the right half-tape. We use $Z_{i,u}$ and $Z_{i,l}$ to refer to the upper and lower half-zones of zone $Z_i$.

- The **control tape** (CT) is a copy of the virtual control tape that the host sees, that is filled in at least from $Z$ through the current head position. The primary role of the control tape is to manage recovery of the head back to $Z$ after it has gone down the tape, but it simplifies reasoning in general.
...
- In addition to the aforementioned properties, the **active counter** (AC) maintains the following invariants, except at $Z$ and when doing constant-time cleanup.
...
- In addition to the aforementioned properties, the **recovering counter** (RC) maintains the following invariants, except at $Z$ and when doing constant-time cleanup.
...
\begin{align*}
  ...
  \kappa'&=\kappa;\\
  \gamma_0'&=\gamma_0,\gamma_1'=\gamma_1,\gamma_{-1}'=\gamma_{-1};\\
  D_c&=0,D_a=0,D_b=0).
\end{align*}
$$
...
\begin{align*}
  ...
  \kappa'&=\kappa;\\
  \gamma_0'&=B,\gamma_1'=\gamma_1,\gamma_{-1}'=\gamma_{-1};\\
  D_c&=0,D_a=0,D_b=0).
\end{align*}
$$
...
- $\gamma_0\in\{B,Z,Z_u,Z_l\}$ denotes the symbol under the head of the control tape. $\gamma_0'$ denotes the symbol to overprint in that position.
...
- The CT head is at $Z$ and sees either of $\gamma_0=B,Z_0$.
...
- The CT head sees $Z_{u,*}$, and is at $Z_{i,u}[0]$. Furthermore, the zone markers from $Z$ through $Z_{i,u}$ are filled in, and the other cells in these zones are $B$.
...
- The state is $\kappa=q_u$.
- The CT head sees $B$, and is at $Z_{i,u}[j]$. Furthermore, the zone markers from $Z$ through $Z_{i,u}$ are filled in, and the other cells in these zones are $B$.
...
- The CT head sees $Z_{l}$, and is at $Z_{i,l}[0]$. Furthermore, the zone markers from $Z$ through $Z_{i,l}$ are filled in, and the other cells in these zones are $B$.
...
- The state is $\kappa=q_l$.
- The CT head sees $B$, and is at $Z_{i,l}[j]$. Furthermore, the zone markers from $Z$ through $Z_{i,l}$ are filled in, and the other cells in these zones are $B$.
...
- when $\kappa=q_{0}$, emit $Z$,
- if $\kappa=q_{u},q_{l}$, emit $\gamma_0$.
- otherwise, emit $B'$, the distinguished blank symbol, signalling incomplete computation.

We now name a specialization of $I_0$. If $I_0$ holds, and additionally $\gamma_0=Z$, we say that $I_0'$ is satisfied.
...
- The tapes are all blank (having the symbol $B$ or $0$),
...
$$
\begin{align*}
\delta[\rho_1=1,-1;\kappa=q_{0}]&=s[d_0'=D_0=\rho_1;\kappa'=p_{0,1};\gamma_0'=Z;\gamma_1'=\gamma_{-1}'=1].
\end{align*}
$$
...
$$
\begin{align*}
\delta[\kappa=p_{0,1};\gamma_0=B]&=s[\kappa'=q_u;\gamma_0'=Z_{u,h}], \\
\delta[\kappa=p_{0,1};\gamma_0=Z_{u,*}]&=s[\kappa'=q_u].
\end{align*}
$$
...
$$
\begin{align*}
\delta[\kappa=p_{u,-1};\gamma_0=Z]&=s[\kappa'=q_0;\gamma_{1}'=\gamma_{-1}'=0].
\end{align*}
$$
